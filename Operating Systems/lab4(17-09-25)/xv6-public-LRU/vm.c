  ...
  for (; a < newsz; a += PGSIZE) {
    ...
    if (curproc) {
      if (curproc->framecount < 16) {
        // Still space left, add frame info
        ...
        // Free victim’s physical memory
        pte_t *vpte = curproc->frames[victim_index].pte;
        uint vpa = PTE_ADDR(*vpte);
        kfree(P2V(vpa));
        *vpte = 0; // Invalidate victim PTE

        // Replace victim entry with new frame
        curproc->frames[victim_index].va = a;
        curproc->frames[victim_index].pte =
            walkpgdir(pgdir, (void*)a, 0);
