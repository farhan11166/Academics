...
int data_block_bitmap[MAX_BLOCKS];
...
int fs_create(const char *name, int creator_id, const char *desc) {
    ...
    // ----------- ADD YOUR CODE (Metadata setup) ----------
    inode_table[inum].creator_id = creator_id;
    inode_table[inum].type = 0; // file
...
int fs_delete(const char *name) {
    ...
    // ------------ ADD YOUR CODE: free all blocks ----------
    for (int j = 0; j < DIRECT_PTRS; j++) {
        int blk = inode_table[inum].direct[j];
        if (blk != -1)
            free_block(blk);
    }
    // -------------------------------------------------------
